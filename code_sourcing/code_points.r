...
data_tab1 <- data_tab1 %>% 	mutate(		
	pt_4_1_tenant_do = case_when( does_the_notification_have_to_be_in_writing %in% 
									c("Yes", "Yes, but email or text is okay" )	~ as.numeric(-1), 
									TRUE ~ 0 ),
...
data_tab1 <- data_tab1 %>% 	mutate(		
	...
	pt_4_4_tenant_do = case_when( when_do_tenants_have_to_provide_documentation %in% 
				c(
					...
					"Within one week of notifying their landlord",
					"Within three business days of receiving a required notice of rent delinquency from the landlord.") ~ -1.5, 
					TRUE ~ 0 ),
...
data_tab1 <- data_tab1 %>% 
	mutate(
		pt_5_1_part_rent = case_when( what_does_the_law_say_about_paying_part_of_the_rent  %in% 
									c( "Tenants must pay as much of the rent as possible" ) ~ -2, 
									TRUE ~ 0),
		...
		pt_5_3_part_rent = case_when( what_does_the_law_say_about_paying_part_of_the_rent %in% 
				c("There shall be a rebuttable presumption that the tenant paid that portion of the rent that the tenant was reasonably able to pay if the tenant paid at least 50% of the monthly rent." , 
					...
					"Housing agency will work with the property management company it contracts with on rent forbearance."  ) ~  1, 
					TRUE ~ 0))
...
data_tab1 <- data_tab1 %>% 
		mutate(
			pt_8_1_fees= case_when(  can_landlords_charge_late_fees_or_interest_on_missed_rent_payments %in% c( "No, they can't charge late fees or interest" )  ~ 3,  
...
data_tab1 <- data_tab1 %>% 
	mutate(
	pt_10_01_court = case_when( are_courts_holding_eviction_proceedings %in% c("Yes, courts have limited proceedings but may still allow evictions") ~  -2,
